# ADR 0019 — GIF / animated disposition: animated → MP4 (HEVC), static → PNG (lossless)

**Date**: 2026-05-31
**Status**: Accepted
**Driven by**: owner decision (2026-05-31); `Docs/ImageBridge-PRD-v0.1.md` §7 + §12
**Depends on**: [ADR-0013](0013-videotoolbox-first-ship-encoder.md) (MP4/HEVC ship encoder), [ADR-0017](0017-still-ship-encoder.md) (PNG/oxipng lossless), [ADR-0016](0016-imagebridge-sibling-package.md) (the bridge handoff)
...
This also pins the single legitimate coupling point between the two packages (ADR-0016): the
...
2. **Static GIF → PNG (lossless).** Decode via ImageIO → optional `FrameProcessor` →
   encode PNG → oxipng (ADR-0017). GIF transparency key maps to PNG alpha. Lossless: pixels
   preserved.
...
- **Reuses both ship encoders, adds none.** Static → ADR-0017 PNG path; animated → ADR-0013
...
- **`ImageBridge` → `FormatBridge` dependency is now load-bearing for the animated path**
  (consistent with ADR-0016's "they meet at sequences"). The static path keeps **no**
...
- [ADR-0013](0013-videotoolbox-first-ship-encoder.md), [ADR-0016](0016-imagebridge-sibling-package.md), [ADR-0017](0017-still-ship-encoder.md)
