# ADR 0016 — ImageBridge as a sibling package (native ImageIO/PDFKit), not a FormatBridge extension
...
handoff is covered by [ADR-0019](0019-gif-and-animated-disposition.md).
...
ImageBridge ──→ ForgeOptimizer (FrameProcessor)        [reused unchanged]
      ...
      └──→ FormatBridge                                [animated/sequence handoff only — ADR-0019]
```
...
- **Zero new optimizer code.** NAFNet/SR/IQA carry over by construction; the AI work is
  already done. The package is I/O + a quality-target search ([ADR-0018](0018-still-quality-target-search.md)).
...
- [ADR-0017](0017-still-ship-encoder.md), [ADR-0018](0018-still-quality-target-search.md), [ADR-0019](0019-gif-and-animated-disposition.md)
