...
// MARK: - Numerical parity (real weights)

/// Validates `loadWeights` against the REAL mlx-community 8-bit checkpoint:
/// the Swift backbone's mean-pooled embedding must track a PyTorch FP reference
/// for an identical input. Gated on the locally-cached 400 MB weights +
/// `parity.safetensors` (generated by the Python ref script), so CI without the
/// cache skips it. This is the test #27 deferred — the one that catches the
/// quant-dequant / key-mapping / transpose bugs the shape-only tests missed.
@Suite("SigLIP2 numerical parity")
struct SigLIP2ParityTests {

    @Test("Swift 8-bit-dequant backbone embedding ≈ PyTorch FP reference (cosine ≥ 0.95)")
    func backboneParity() throws {
        try withCPU {
            let cache = SigLIP2BackboneLoader.defaultCacheRoot
            let weightsURL = cache.appending(path: "model.safetensors")
            let parityURL = cache.appending(path: "parity.safetensors")
            guard FileManager.default.fileExists(atPath: weightsURL.path),
                  FileManager.default.fileExists(atPath: parityURL.path) else {
                print("[parity] cached weights/parity fixture absent → skipping (run the Python ref script locally)")
                return
            }

            let fixture = try MLX.loadArrays(url: parityURL)
            guard let input = fixture["input_nhwc"], let ref = fixture["ref_embedding"] else {
                Issue.record("parity.safetensors missing input_nhwc / ref_embedding"); return
            }

            let model = SigLIP2VisionModel()
            try model.loadWeights(from: weightsURL)
            let pooled = model(input).poolerOutput.asType(.float32).reshaped([-1])  // [768]
            let refV = ref.asType(.float32).reshaped([-1])

            let dot = MLX.sum(pooled * refV)
            let cos = (dot / (MLX.sqrt(MLX.sum(pooled * pooled)) * MLX.sqrt(MLX.sum(refV * refV))))
            let maxAbs = MLX.max(MLX.abs(pooled - refV))
            MLX.eval(cos, maxAbs)
            let cosV: Float = cos.item(Float.self)
            let maxAbsV: Float = maxAbs.item(Float.self)
            print(String(format: "[parity] cosine=%.4f  maxAbsDiff=%.4f", cosV, maxAbsV))

            #expect(cosV >= 0.95,
                    "Backbone embedding cosine \(cosV) < 0.95 vs PyTorch FP — loadWeights still wrong")
        }
    }
}
