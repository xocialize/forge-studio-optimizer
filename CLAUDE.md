...
| 0017 | **AV1 tier via SVT-AV1 (#52 Phase A subprocess, #58 Phase B in-process — DONE)** — no HW AV1 encode on Apple Silicon (VT −12908), so SVT-AV1 software. `forge-quality-target --codec av1` (VMAF-targeted CRF search + `--film-grain`). AV1 ~44–53% < HEVC on signage. **Phase B done**: SVT-AV1 2.3.0 (BSD) built into FFmpegXC + `FormatBridge.FFmpegAV1Encoder` (in-process decode→libsvtav1→mp4, BT.709) → self-contained, no ffmpeg subprocess (VMAF measurement stays external dev-tool) | Accepted |
...
- **Validate the RIGHT pixels — corners/edges, not just center (ImageBridge PDF, 8th catch)**: `CGPDFPage.getDrawingTransform(_:rect:rotate:preserveAspectRatio:)` only scales a page **down** to fit — handed a pixel rect *larger* than the page (any DPI > 72, incl. the default 150) it places the page 1:1 and **centers it, leaving white margins**. So PDF rasterization above 72 DPI rendered the artwork at point-size in a white sea. It hid because PDFTests sampled only **center** pixels (content either way) + asserted output dims (correct). Fix: scale points→pixels yourself (`ctx.scaleBy(dpi/72)`) and use `getDrawingTransform` only for a **point-sized (1:1)** rotation/origin fit. The test that catches it asserts the four **corners** are the fill color at 288 DPI. Same family as the NV12/VMAF/blockiness catches: a center-only check passes while the frame is wrong.
