...
- **Per-shot is not a free win**: splitting into per-shot encodes adds an IDR keyframe + rate-control warmup per segment; per-shot only beats per-title when shot-complexity diversity (easy shots → much lower quality) exceeds that overhead AND shots are long enough to amortize it. A bounded sample with one dominant shot *loses* (-18%). Need diverse/long-shot content or split-only-when-it-pays logic.
